...
// src/getWorkspacesFromInput.ts
var import_core = __toESM(require_core());
var getWorkspacesFromInput = () => {
  const input = (0, import_core.getInput)("workspaces");
  if (!input)
    return null;
  try {
...
// src/getChangedWorkspace.ts
var import_minimatch = __toESM(require_minimatch());

// src/utils.ts
var import_path = require("path");
var splitPath = (path2) => path2.split(import_path.posix.sep);

// src/getWorkspacePath.ts
var import_path2 = __toESM(require("path"));
var getWorkspacePath = (file, workspace) => {
  ...
  return import_path2.default.posix.join(...output);
};

// src/getChangedWorkspace.ts
var getChangedWorkspace = (file, workspace) => {
  if ((0, import_minimatch.default)(file, workspace)) {
    return getWorkspacePath(file, workspace);
  }
  const splittedFile = splitPath(file);
  const splittedPattern = splitPath(workspace);
  for (let i = 0; i < splittedPattern.length; ++i) {
    if (splittedFile[i] !== splittedPattern[i])
      return null;
  }
  return workspace;
};
