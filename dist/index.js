...
var require_read_package_json_fast = __commonJS({
  "node_modules/read-package-json-fast/index.js"(exports, module2) {
    ...
    var parse2 = require_json_parse_even_better_errors();
    var { resolve, dirname, join, relative } = require("path");
    var rpj = (path3) => readFile(path3, "utf8").then((data) => readBinDir(path3, normalize(stripUnderscores(parse2(data))))).catch((er) => {
      er.path = path3;
      throw er;
    });
...
var getChangedFiles = async () => {
  if (import_github.context.eventName === "pull_request") {
    return getChangedFilesFromGit(import_github.context.payload.pull_request.base.ref);
...
var import_map_workspaces = __toESM(require_lib4());
var import_yaml = __toESM(require_dist2());
var getPackageConfig = async (packageDirPath) => {
  ...
  if (!parsedConfig.workspaces) {
    const pnpmWorkspacesPath = import_path.default.join(packageDirPath, "pnpm-workspace.yaml");
    if (import_fs.default.existsSync(pnpmWorkspacesPath)) {
      const file = await import_fs.default.promises.readFile(pnpmWorkspacesPath);
      const pnpmWorkspacesConfig = (0, import_yaml.parse)(file.toString());
...
var getWorkspaces = async () => {
  const packageDirPath = (0, import_core2.getInput)("working-directory") || process.cwd();
...
var run = async () => {
  ...
  (0, import_core3.setOutput)("packages", packages);
  (0, import_core3.setOutput)("empty", packages.length === 0);
};

// src/index.ts
run().catch((err) => (0, import_core4.setFailed)(err));
/*!
