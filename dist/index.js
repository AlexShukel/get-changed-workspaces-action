...
var isMatches = (file, pattern) => {
  if ((0, import_minimatch.default)(file, pattern)) {
    return true;
  }
  const splittedFile = file.split(import_path.default.posix.sep);
  const splittedPattern = pattern.split(import_path.default.posix.sep);
...
var run = async () => {
  ...
  const output = [];
  const token = (0, import_core2.getInput)("token");
